...
def get_lower_to_upper_dict():
    ...
    """
    '''dict1=dict(zip(map(chr,range(97,123)),range(1,27)))
    dict2=dict(zip(map(chr,range(65,91)),range(1,27)))
    dict1.update(dict2,);'''
    return dict(zip(map(chr,range(97,123)),map(chr,range(65,91))))
...
def capitalize_names(input):
    ...
    return list(map(lambda x:str.capitalize(x),map(lambda x:str.lower(x),input)))
